...
-- BUSQUEDA POR NOMBRE DE LIBRO
 ...
 -- BUSQUEDA POR NOMBRE DE AUTOR
  IF p_1=2 THEN
   select l.nombre,inv.id as idInventario,inv.numeroInventario 
	...
	where a.nombre like concat('%', p_2, '%') AND inv.estadoMaterial!='Prestado' AND inv.eliminado!=1;
 END IF;
 ...
 IF p_1=3 THEN
   select l.nombre,iv.id as idInventario,iv.numeroInventario 
	from libro l 
    inner join inventario iv on iv.idLibro = l.id 
where isbn like concat(p_2, '%') AND iv.estadoMaterial!='Prestado' AND inv.eliminado!=1;
 END IF;
END $$
DELIMITER ;
...
CREATE PROCEDURE `bibliotecauae`.`guardarDocumento`(in p_nombre varchar(40),in p_cantidadPaginas int,
in p_informacionAdicional varchar(25),in p_terminosResumen varchar(40),in p_numeroEdicion int,
in p_referenciaDigital varchar(50),in p_fechaIso date,in p_fechaPublicacion date,in p_idioma varchar(15),
...
in p_precio varchar(40),in p_facilitante varchar(40),in p_solicitante varchar (50),in p_entrego varchar(40),
in p_fechaEntrega date)
BEGIN
INSERT INTO `libro` 
(`nombre`, `cantidadPaginas`,
 `informacionAdicional`, `terminosResumen`, `numeroEdicion`,
 `referenciaDigital`, `fechaIso`, `fechaPublicacion`, `idioma`,
 ...
 p_referenciaDigital, p_fechaIso, p_fechaPublicacion, p_idioma,
...
`precio`, `facilitante`, `estadoMaterial`, `fechaEstado`, `solicitante`, `entrego`, `fechaEntrega`,`eliminado`) 

VALUES (NULL, p_numeroInventario, @idLibro, p_fechaAdquisicion, p_volumen, p_formaAdquisicion, p_precio, 
p_facilitante, 'Disponible', curdate(), p_solicitante,p_entrego, p_fechaEntrega,0);
...
CREATE PROCEDURE `bibliotecauae`.`infoLibro`()
	BEGIN
select l.id,iv.numeroInventario,l.nombre,l.numeroEdicion,l.idioma,e.nombre as editorial,p.nombre as pais,tCol.nombre as tipoColeccion,
